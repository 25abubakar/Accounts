...
{
    /// <inheritdoc />
    public partial class AddIsBlockedToChatConversationsSP : Migration
    {
        /// <inheritdoc />
...
BEGIN
    ...
    SELECT 
        ...
        CAST(
            CASE 
                WHEN c.ConversationType = 'Direct' AND EXISTS (
                    SELECT 1 FROM ChatBlocks b
                    WHERE b.BlockerPersonId = @PersonId AND b.BlockedPersonId = otherMember.PersonId
        ...
        CAST(
            CASE 
                WHEN c.ConversationType = 'Direct' AND EXISTS (
                    SELECT 1 FROM ChatBlocks b
                    WHERE b.BlockerPersonId = otherMember.PersonId AND b.BlockedPersonId = @PersonId
                ) THEN 1 ELSE 0 END AS BIT
        ) AS IsBlockedByOther
