/*
    Emergency repair for accidental attendance auto-locks.

    Source of the login error:
    - Frontend LoginPage displays the API message.
    - AuthService accepts the password, then AccountScopeAccessService rejects the user.
    - AccountScopeAccessService returns: "This account is disabled or locked."
    - The bad lock was created by dbo.usp_Attendance_EvaluateStatuses writing to
      AspNetUsers.LockoutEnd and Persons.IsActive during attendance report evaluation.

    This script restores login for users in active tenants and patches the evaluator
    so attendance reports cannot directly lock application login accounts again.
*/
...
SELECT
    RestoredUsers = COUNT_BIG(1)
FROM dbo.AspNetUsers identityUser
WHERE identityUser.LockoutEnd IS NULL;
