...
public sealed class StatusConfigurationsController : ControllerBase
{
    ...
    {
        var rows = await _db.ProcessStatusStyles.AsNoTracking()
        .Include(x => x.Process).Include(x => x.Status).Include(x => x.ColorStyle)
        .OrderBy(x => x.Process.ProcessName).ThenBy(x => x.DisplayOrder)
        .ToListAsync(ct);
    ...
    public async Task<IActionResult> Create(StatusConfigurationWriteDto dto, CancellationToken ct)
    {
        ...
        var entity = new ProcessStatusStyle { CreatedDate = DateTime.UtcNow };
    ...
    {
        if (!CanManage()) return Forbid();
        var entity = await _db.ProcessStatusStyles.FirstOrDefaultAsync(x => x.Id == id, ct);
    ...
    private static StatusConfigurationDto ToDto(ProcessStatusStyle x)=>new(x.Id,x.Process.ProcessName,x.Status.StatusName,x.Code,x.Description,x.ColorStyle.ColorName,x.ColorStyle.ColorCode,x.ColorStyle.FontColor,x.ColorStyle.FontSize,x.DisplayOrder,x.IsPaid,x.IsActive);
    private bool CanManage()=>_tenant.IsSuperAdmin||_tenant.IsTenantAdmin;
}

public sealed record StatusConfigurationDto(int Id,string ProcessName,string StatusName,string Code,string? Description,string ColorName,string ColorCode,string FontColor,string FontSize,int DisplayOrder,bool IsPaid,bool IsActive);
