...
eranceMinutes,
   ...
   FROM dbo.AttendancePolicies
    ...
    IF @PolicyId IS NULL
        THROW 51000, 'No active attendance policy is config
ured.', 1;
...
ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;
    ...
    FROM dbo.ProcessStatusStyles pss
    INNER JOIN PlatformSettings.StatusCrDbValues crdb ON crdb.DbValue = pss.Code AND (crdb.TenantId = @TenantId
 OR crdb.TenantId IS NULL)
    INNER JOIN PlatformSettings.ActionStatuses pas ON pas.StatusId = crdb.StatusId AND pas.ActionId=@PlatformActionId
    WHERE pss.ProcessId = @ProcessId AND pss.Code = N'A' AND pss.IsActive = 1 AND (pss.TenantId = @TenantId OR
 pss.TenantId IS NULL)
    ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;

    SELECT TOP(1) @PlatformLate = pas.Id
    FROM dbo.ProcessStatusStyles pss
    INNER JOIN PlatformSettings.StatusCrDbValues crdb ON crdb.DbValue = pss.Code AND (
crdb.TenantId = @TenantId OR crdb.TenantId IS NULL)
    INNER JOIN PlatformSettings.ActionStatuses pas ON pas.StatusId = crdb.StatusId AND pas.ActionId=@PlatformActionId
    WHERE pss.ProcessId = @ProcessId AND pss.Code = N'LT' AND pss.IsActive = 1 AND (p
ss.TenantId = @TenantId OR pss.TenantId IS NULL)
    ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;

    SELECT TOP(1) @PlatformCompletedLate = pas.Id
    FROM dbo.ProcessStatusStyles pss
    INNER JOIN PlatformSettings.StatusCrDbValues cr
db ON crdb.DbValue = pss.Code AND (crdb.TenantId = @TenantId OR crdb.TenantId IS NULL)
    INNER JOIN PlatformSettings.ActionStatuses pas ON pas.StatusId = crdb.StatusId AND pas.ActionId=@PlatformActionId
    WHERE pss.ProcessId = @ProcessId AND pss.Code 
= N'TP' AND pss.IsActive = 1 AND (pss.TenantId = @TenantId OR pss.TenantId IS NULL)
    ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;

    SELECT TOP(1) @PlatformShortLeave = pas.Id
    FROM dbo.ProcessStatusStyles pss
    INNER JOIN Plat
formSettings.StatusCrDbValues crdb ON crdb.DbValue = pss.Code AND (crdb.TenantId = @TenantId OR crdb.TenantId IS NULL)
    INNER JOIN PlatformSettings.ActionStatuses pas ON pas.StatusId = crdb.StatusId AND pas.ActionId=@PlatformActionId
    WHERE pss.Proc
essId = @ProcessId AND pss.Code = N'SL' AND pss.IsActive = 1 AND (pss.TenantId = @TenantId OR pss.TenantId IS NULL)
    ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;
...
StatusStyles pss
    INNER JOIN PlatformSettings.StatusCrDbValues crdb ON crdb.DbValue = pss.Code AND (crdb.TenantId = @TenantId OR crdb.TenantId IS NULL)
    INNER JOIN PlatformSettings.ActionStatuses pas ON pas.StatusId = crdb.StatusId AND pas.ActionId=
@PlatformActionId
    WHERE pss.ProcessId = @ProcessId AND pss.Code = N'EL' AND pss.IsActive = 1 AND (pss.TenantId = @TenantId OR pss.TenantId IS NULL)
    ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;

    SELECT TOP (1) @DayOff = pss.Id
, @PlatformDayOff = pas.Id
    FROM dbo.ProcessStatusStyles pss
    INNER JOIN PlatformSettings.StatusCrDbValues crdb ON crdb.DbValue = pss.Code AND (crdb.TenantId = @TenantId OR crdb.TenantId IS NULL)
    INNER JOIN PlatformSettings.ActionStatuses pas ON
 pas.StatusId = crdb.StatusId AND pas.ActionId=@PlatformActionId
    WHERE pss.ProcessId = @ProcessId AND pss.Code = N'DO' AND pss.IsActive = 1 AND (pss.TenantId = @TenantId OR pss.TenantId IS NULL)
    ORDER BY CASE WHEN pss.TenantId = @TenantId THEN 0 E
LSE 1 END;
...
INNER JOIN PlatformSettings.ActionStatuses pas ON pas.StatusId = crdb.StatusId AND pas.ActionId=@PlatformActionId
    ...
    ORD
ER BY CASE WHEN pss.TenantId = @TenantId THEN 0 ELSE 1 END;
...
sonId = previousAbsent.PersonId
                  ...
                  AND previo
usAbsent.AttendanceDate < dates.AttendanceDate
            ) AS PriorMonthlyAbsentCount
        FROM dbo.Persons person
        JOIN dbo.StaffVacancy staff ON staff.PersonId = person.PersonId
        JOIN dbo.AttendanceMapRules mapRule ON mapRule.StaffId 
= staff.StaffId AND mapRule.TenantId = @TenantId
        JOIN PlatformTypes.AttendanceTypes entryType ON entryType.Id = mapRule.AttendanceEntryTypeId AND entryType.IsActive = 1 AND entryType.TenantId = @TenantId
...
iming.StaffId = staff.StaffId
 ...
 AttendanceDate
        ...
        WHERE effective.IsOn = 1
          ...
          AND effective.PriorMonthlyAbsentCount >= effective.AdjustAbs
entDays
          AND @AsOfUtc > DATEADD(
                minute,
    ...
    INSERT dbo.AttendanceRecords
        (TenantId, PersonId, AttendanceDate, AttendanceStatusId, Platf
ormActionStatusId, TotalBreakMinutes, CreatedDate, ModifiedDate)
...
tusId =
           CASE
               WHEN effective.IsOn = 0 AND attendance.CheckInUtc IS NULL THEN
                   CASE
                       WHEN timingHoliday.ValueCode IN (N'HOLIDAY', N'ANNUAL_HOLIDAY') OR DATENAME(weekday, attendance.Attendance
Date) = N'Sunday'
                           THEN COALESCE(@Holiday, @DayOff, attendance.AttendanceStatusId)
                       ELSE COALESCE(@DayOff, @Holiday, attendance.AttendanceStatusId)
                   END
               WHEN ISNULL(mapRule.I
sOpenAttendance, 0) = 1 AND attendance.CheckInUtc IS NOT NULL THEN @Present
               ...
               WHEN attendance.CheckOutUtc IS NULL AND @AsOfUtc > effective.MissingCheckou
tDeadline THEN @Absent
               ...
               WHEN attendance.CheckOutUtc < attenda
nce.CheckInUtc THEN @Absent
               WHEN attendance.CheckOutUtc > effective.MissingCheckoutDeadline THEN @Absent
               WHEN DATEDIFF(minute, attendance.CheckOutUtc, effective.ShiftEndLocal) > effective.EarlyCheckoutAbsentAfterMinutes THEN 
@Absent
               ...
               WHEN attendance.
CheckInUtc > effective.OnTimeDeadline THEN @CompletedLate
               ELSE @Present
           END,
           ...
           CASE
               WHEN effective.IsOn = 0 AND attendance.CheckInUtc IS NULL THEN
                   CAS
E
           WHEN timingHoliday.ValueCode IN (N'HOLIDAY', N'ANNUAL_HOLIDAY') OR DATENAME(weekday, attendance.AttendanceDate) = N'Sunday'
   ...
   WHEN attendance.AttendanceStatusId = @ShortLeave THEN @PlatformShortLeave
               ...
               WHEN attendance.CheckO
utUtc IS NULL AND attendance.CheckInUtc > effective.OnTimeDeadline THEN @PlatformLate
   ...
   WHEN DATEDIFF(minute, attendance.CheckInUtc, attendance.CheckOutUtc) - ISNULL(attendance.TotalBreakMinutes, 0) < effective.RequiredWorkingMinutes - effective.CheckOutAdjustMinutes THEN @PlatformEarlyDeparture
               WHEN attendance.CheckInUtc >
 effective.OnTimeDeadline THEN @PlatformCompletedLate
           ...
           CASE
             ...
             CASE
                       WHEN timingHoliday.ValueCode IN (N'HOLIDAY', N'ANNUAL_HOLIDAY') OR DATENAME(weekday, attendance.AttendanceDate) = N'Sunday'
                           THEN COALESCE(@PlatformHoliday, @PlatformDayOff, attendance.Cam
eraPlatformActionStatusId)
                       ELSE COALESCE(@PlatformDayOff, @PlatformHoliday, attendance.CameraPlatformActionStatusId)
                   END
               WHEN ISNULL(mapRule.IsOpenAttendance, 0) = 1 AND attendance.CameraCheckInUtc 
IS NOT NULL THEN @PlatformPresent
               WHEN attendance.AttendanceStatusId = @ShortLeave THEN @PlatformShortLeave
               ...
               WHEN attendance.CameraCheckOutUtc IS NULL AND @AsOfUtc > effective.MissingCheck
outDeadline THEN @PlatformAbsent
               WHEN attendance.CameraCheckOutUtc IS NULL AND attendance.CameraCheckInUtc > effective.OnTimeDeadline THEN @PlatformLate
       ...
       WHEN attendance.CameraCheckOutUtc < attendance.CameraCheckInUtc THEN @PlatformAbsent
               ...
               WHEN DATEDIFF(minute, attendance.CameraChe
ckOutUtc, effective.ShiftEndLocal) > effective.EarlyCheckoutAbsentAfterMinutes THEN @PlatformAbsent
               WHEN DATEDIFF(minute, attendance.CameraCheckInUtc, attendance.CameraCheckOutUtc) - ISNULL(attendance.TotalBreakMinutes, 0) < effective.Requi
redWorkingMinutes - effective.CheckOutAdjustMinutes THEN @PlatformEarlyDeparture
...
dDate = @AsOfUtc
    ...
    LEFT JOIN dbo.AttendanceMapRules mapRule ON mapRul
e.StaffId = staff.StaffId AND mapRule.TenantId = @TenantId
    LEFT JOIN dbo.AttendanceRuleSettings setting
      ON setting.TenantId = @TenantId
     AND setting.AttendanceEntryTypeId = mapRule.AttendanceEntryTypeId
     AND setting.IsActive = 1
   ...
   CROSS APPLY
    (
        SELECT
            ...
            DATEADD(minute, COALESCE(setting.MissingCheckoutAfterShiftEndMinutes, @MissingOutAfter), windows.Shif
tEndLocal) AS MissingCheckoutDeadline,
