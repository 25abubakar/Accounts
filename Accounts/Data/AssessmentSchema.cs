...
public static class AssessmentSchema
{
    ...
    public static async Task EnsureCurrentAsync(ApplicationDbContext db)
    {
        ...
        {
            if (_ready) return;
            // SQL Server compiles a complete batch before executing ALTER TABLE.
            // Keep every new column in its own command so later statements are
            // compiled only after the column physically exists.
            ...
            await db.Database.ExecuteSqlRawAsync("""
                ...
                """, CancellationToken.None);
            // Rating is part of the legacy monthly-rank index. SQL Server will not
            // alter the column until that dependent index has been removed. Keep
            // these as separate commands because SQL Server compiles a batch before
            // executing it and would otherwise still see the old dependency.
            await db.Database.ExecuteSqlRawAsync("""
                IF EXISTS
                (
                    SELECT 1
