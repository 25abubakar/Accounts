...
BEGIN
    ...
    StaffRows AS (
        SELECT 
            ...
            jobTitle.TitleName AS JobTitle,
            department.Name AS Department,
            assessment.CurrentPay,
        ...
        LEFT JOIN dbo.Departments department ON department.Id = vacancy.DepartmentId
        LEFT JOIN dbo.StaffAssessments assessment ON assessment.VacancyId = vacancy.VacancyId AND assessment.IsActive = 1
        LEFT JOIN dbo.AttendanceRuleSettings ars ON ars.TenantId = @TenantId
        WHERE person.TenantId = @TenantId 
          AND person.IsActive = 1
          AND (@VisiblePersonIds IS NULL OR person.PersonId IN (SELECT PersonId FROM VisiblePeople))
    ),
    BaseRows AS (
        SELECT 
            s.*,
