...
content = content.replace(
  "const [form, setForm] = useState<Partial<DeductionForm>>({});",
  "const [form, setForm] = useState<Partial<DeductionForm>>({});\n  const [showApprovalModal, setShowApprovalModal] = useState(false);\n  const [approvalRow, setApprovalRow] = useState<any>(null);\n  const [pinCode, setPinCode] = useState<string>(\"\");"
);

const saveAdjustmentRegex = /(const handleAdjustmentSubmit = async \(\) => \{.*?\n  \};)/s;
const approveFunction = `
  const handleApproveSubmit = async () => {
    if (!approvalRow || !pinCode) return;
    try {
      await attendanceApi.approveAdjustment(approvalRow.personId, year, month, parseInt(pinCode));
      ...
      toast.error(e?.response?.data?.message || "Failed to approve adjustment.");
    }
  };
`;
...
content = content.replace(
  "onDoubleClick={() => {",
  "onDoubleClick={() => {\n          if (row.original.isAdjustmentApproved) {\n            toast.info(\"Approved adjustment cannot be modified.\");\n            return;\n          }"
);
...
`;
content = content.replace(/<\/div>\s*<Modal\s+isOpen=\{showForm\}/, modalJsx + "\n      <Modal isOpen={showForm}");
